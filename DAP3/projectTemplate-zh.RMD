...
```{r echo=FALSE, Bivariate_Plots}
plot1 <- ggplot(aes(x = free.sulfur.dioxide, y = quality), data = pf) +
  geom_point(alpha = 0.1) + 
  ggtitle('free.sulfur.dioxide')
plot2 <- ggplot(aes(x = combined.sulfur.dioxide, y = quality), data = pf) +
  geom_point(alpha = 0.1) + 
  ggtitle('combined.sulfur.dioxide')
plot3 <- ggplot(aes(x = total.sulfur.dioxide, y = quality), data = pf) +
  geom_point(alpha = 0.1) + 
...
```{r echo=FALSE, Bivariate_Plots}
cor.test(pf$combined.sulfur.dioxide, pf$quality, method = 'pearson')
```
...
## 挥发性酸度、柠檬酸度、固定酸和质量的关系

```{r echo=FALSE, Multivariate_Plots}
library(reshape2)
ggplot(aes(x = volatile.acidity, y = citric.acid), data = pf) +
  geom_point(aes(color = fixed.acidity), alpha = 0.1, size = 0.5) +
  scale_x_continuous(breaks = seq(0, 1.6, 0.1)) +
...
ggplot(aes(x = citric.acid, y = log10(fixed.acidity)), data = pf) +
  geom_point(aes(color = pH), alpha = 0.1, size = 0.5) +
  facet_wrap(~quality, ncol = 2)
```
```{r echo=FALSE, Multivariate_Plots}
ggplot(aes(x = citric.acid, y = fixed.acidity), data = pf) +
  geom_point(aes(color = pH), alpha = 0.3, size = 0.5) +
  facet_wrap(~quality, ncol = 2)
```
...
ggplot(aes(x = quality, y = citric.acid), data = pf) +
  ...
  geom_line(method = 'lm', stat = 'summary', fun.y = mean)
```
